...
@section('content')
    <section id="__TargetArticle__" class="__target__article__ layout-maxed">
        <div class="__article__">
            <div class="__article-nb{{ $ARTICLE->identifier }}__" data-article="{{ $ARTICLE->identifier }}"
                data-url="{{ $ARTICLE->UrlArticle }}">
                <blockquote>
                ...
                <div class="__article__informations__">
                    <time data-time="{{ date($ARTICLE->UploadDate) }}"> Published on
                        {{ date('d/m/Y', strtotime(date($ARTICLE->UploadDate))) }}</time>
                    {{ $ARTICLE->author ? ',' . $ARTICLE->author : '' }}
                </div>
                <main class="__article__center__presentation__">
                    <figure class="__article__image__">
                        <picture>
                            <img src="{{ $ARTICLE->LinkImage }}"
                                alt="{{ $ARTICLE->AltImage === 'null' ? '' : $ARTICLE->AltImage ?? '' }}"
                                title="{{ $ARTICLE->title }}">
                        </picture>
                        <figcaption>
