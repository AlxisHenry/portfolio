...
const ElementToMoveDuringExtends = Array.from(
  document.querySelectorAll("section")
); // Extends all sections on the page
const AddElementsToExtendsMethod = (elements) => {
  elements.forEach((element) => {
    ElementToMoveDuringExtends.push(element);
  });
};
const AdditionalsElements = [
  // Add additionals elements to extends them
...
export const ResizeNavAnimation = () => {
...
export const HoverNavbarMenu = () => {
  // Hover navbar elements
  const NavTitle = document.querySelectorAll(
    ".burger-element a:not(.nav-active)"
  );
  if (NavTitle) {
